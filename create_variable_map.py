...
def create_dataset(project_id: str, dataset:str, cf:bool = True):
    ...
    for source in dataset_config[dataset]["source"]:
        for file in files:
            if f"_{source}_" in os.path.basename(file):
                ds = xr.open_dataset(file)
    ...
    if cf:
        ...
        data["variables"]["latitude"]["attributes"]["_CoordinateAxisType"] = {"type": "string", "data": "Lat"}
        data["variables"]["latitude"]["attributes"]["axis"] = {"type": "string", "data": "Y"}
        data["variables"]["latitude"]["attributes"]["ioos_category"] = {"type": "string", "data": "Location"}

        data["variables"]["longitude"]["attributes"]["_CoordinateAxisType"] = {"type": "string", "data": "Lon"}
        data["variables"]["longitude"]["attributes"]["axis"] = {"type": "string", "data": "X"}
        data["variables"]["longitude"]["attributes"]["ioos_category"] = {"type": "string", "data": "Location"}
    ...
    data["attributes"]["infoUrl"] = {"type": "string", "data": "https://saga.pmel.noaa.gov/data/index.html"}
    ...
    data["variables"]["duration"]["data"] = list(ds[variable_names["duration"]["old_name"]].values * 60.)

    try:
        mid = []
        end = []
        for dts, dur in zip(data["variables"]["time"]["data"], data["variables"]["duration"]["data"]):
    ...
    for source, ds in source_data.items():
        for name, var in data["variables"].items():
            ...
            if name in ["time", "trajectory_id", "duration"]:
                continue
            try:
                data["variables"][name]["data"] = list(ds[variable_names[name]["old_name"]].values)
            except KeyError as e:
    ...
    os.makedirs(output_path, exist_ok=True)
    with open(os.path.join(output_path,f"{project}_{platform}_{dataset}_v{product_version}.json"), "w") as f:
        json.dump(data, f)

    with open(os.path.join(output_path,f"{project}_{platform}_{dataset}_v{product_version}.csv"), "w") as f:
...
if __name__ == "__main__":
    ...
    # for proj in ["ATOMIC", "CALNEX", "NAAMES4", "NAAMES3", "NAAMES2", "NAAMES1"]:
    for proj in ["NAAMES2", "NAAMES1"]:
